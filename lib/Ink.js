    ...
    _this._onPress = function (event) {
      ...
      if (changedTouches) {
        for (var index = 0; index < changedTouches.length; index++) {
          _this.pushBlot(timeStamp, changedTouches[index].clientX, changedTouches[index].clientY);
        }
      } else if (button === MOUSE_LEFT && !ctrlKey) {
        _this.pushBlot(timeStamp, clientX, clientY);
      }
    };

    _this._onRelease = function () {
      _this.state.store.release(Date.now());
    };
